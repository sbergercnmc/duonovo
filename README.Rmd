...
The following shows how to run duoNovo, assuming we have a VCF "duo_proband_father.longread.hiphase.vcf.gz" from long-read sequencing, and a VCF "duo_proband_father.shortRead.vcf.gz" from short-read sequencing.
...
- **candidate_variants_concordant_with_SRS**: Logical value specifying if candidate variants should be concordant with short-read sequencing.
- **SRS_vcf_file_path**: File path to the VCF containing variant calls from short-read sequencing of the duo.
- **test_reference_allele**: Logical value specifying if positions where the proband is heterozygous and the parent is homozygous for the variant allele should also be tested (default is `FALSE`).
...
- **output_vcf_path**: File path to be used when writing the output VCF file.
