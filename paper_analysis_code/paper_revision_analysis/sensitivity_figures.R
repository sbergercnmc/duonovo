...
if (length(args) != 2) {
  stop("Usage: Rscript sensitivity_figures.R <data_directory> <figure_directory>")
}
...
trio_dn_count_before_ad_filter <- sapply(1:length(all_dirs), function(xx) {
  setwd(all_dirs[xx])
  all_data <- list.files()
  load(file = all_data[grep("sensitivity", all_data)])
  sens_both_no_ad_filter['total']
