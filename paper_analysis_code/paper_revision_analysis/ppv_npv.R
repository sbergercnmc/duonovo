...
rownames(ppv_pf) <- rownames(ppv_pf_no_gnomad) <- rownames(ppv_pm) <- rownames(ppv_pm_no_gnomad) <- c('dn', 'assessed', 'total')
...
### PPV plots 
pdf(file = paste0(figure_directory, "/ppv_no_gnomad.pdf"), height = 2.4, width = 4.8, pointsize = 8)
par(mfrow = c(1, 2))
...
plot(sort(precision_pf, decreasing = TRUE), ylim = c(0, 1), pch = 19, col = "dark orange", xlab = "Duo index", 
     ylab = "PPV (PF)", main = "Variants absent from gnomAD", cex = 0.75)
...
plot(sort(precision_pm, decreasing = TRUE), ylim = c(0, 1), pch = 19, col = "dark orange", xlab = "Duo index", 
     ylab = "PPV (PM)", main = "Variants absent from gnomAD", cex = 0.75)
dev.off()

pdf(file = paste0(figure_directory, "/ppv_n_called_pf.pdf"), 
...
pdf(file = paste0(figure_directory, "/ppv_n_called_pm.pdf"), 
       height = 1.5, width = 2.2, pointsize = 8)
...
### NPV plots
pdf(file = paste0(figure_directory, "/father_proband_npv.pdf"), 
    height = 2.4, width = 6, pointsize = 8)
...
axis(2, at = c(0.999, 1))
abline(v = seq(1.5, 116.5, by = 1), col = rgb(0,0,0,0.4), lty = "longdash")
...
pdf(file = paste0(figure_directory, "/mother_proband_npv.pdf"), 
       height = 2.4, width = 6, pointsize = 8)
