...
#SBATCH --output=calculate_coverage_%A_%a.out
#SBATCH --error=calculate_coverage_%A_%a.err
...
total_dirs=${#directories[@]}
  ...
  # Function to calculate average coverage
  calculate_coverage() {
    local bam_file="$1"
    samtools depth -@ "$SLURM_CPUS_PER_TASK" -a "$bam_file" | \
  ...
  output_file="${OUTPUT_DIR}/${dir}_coverage.tsv"
  echo -e "Sample\tRelationship\tAverage Coverage" > "$output_file"
  ...
  if [ -f "$bam_path" ]; then
  avg_coverage=$(calculate_coverage "$bam_path")
  echo -e "${id}\t${relationship}\t${avg_coverage}" >> "$output_file"
