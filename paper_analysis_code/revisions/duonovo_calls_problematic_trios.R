...
###
### now among non-problematic trios
...
n_denovo_pf <- rep(NA, length(all_dirs_no_problematic))
n_denovo_pm <- n_denovo_pf
...
pdf(file = paste0(figure_directory, "/duonovo_non_problematic_trios.pdf"), height = 2.2, width = 2.2, pointsize = 8)
plot(n_denovo_pf + n_denovo_pm, trio_dn_count, xlim = c(0, 90), ylim = c(0, 90), col = "darkorange", 
     cex = 0.5, bty = 'l', xaxt = 'n', yaxt = 'n', 
     xlab = "# de novo variants (duoNovo)", ylab = "# de novo variants (full trio genotyping)")
abline(0, 1, col = rgb(0,0,0,0.5), lty = "longdash")
axis(1, at = c(0, 40, 80))
